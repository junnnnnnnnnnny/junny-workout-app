// 사용자가 캡처해 준 맥도날드 코리아 영양성분표를 공통 음식 DB(foods 컬렉션)에 등록하는 1회성 스크립트.
// 표에 포화지방/당만 있고 총 탄수화물/총 지방 수치가 없어 carbsG/fatG는 0으로 둠(추후 보완 필요).
//
// 실행 방법 (로컬에서, 실제 값이 채워진 .env.local로):
//   node --env-file=.env.local scripts/seed-mcdonalds-foods.mjs
...
].map(([name, unit, calories, proteinG]) => ({
  name,
  unit,
  calories,
  proteinG,
...
for (let i = 0; i < items.length; i += batchSize) {
  ...
  await batch.commit();
  written += Math.min(batchSize, items.length - i);
